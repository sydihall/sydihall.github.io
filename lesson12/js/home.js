window.addEventListener('load', async ()=> {
    ...
    for (let i=0; i<townsObject.length; i++) {
        console.log(townsobject[i]);
        if (townsObject[i].name in towns) {
            const selector = towns[townsObject[i].name];
            document.querySelector(`section.${selector} h2`).innerHTML = townsObject[i].name
            document.querySelector(`section.${selector} h3`).innerHTML = townsObject[i].motto
            ...
            document.querySelector(`section.${selector} p.rainfall`).innerHTML = `Rain Faill: ${townsObject[i].averageRainfall}`
